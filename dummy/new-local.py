...
def main( args ):
    ...
    for batch in batches:
        ...
        for k in featmaps:
            ...
            if PLAT is 'cpu':
                cmd = 'OPENBLAS_NUM_THREADS=%s ./dummy --gpu 0 --network %s --layer_csv %s --trial 1000000' % (THREADS, NET, OUTNAME)
            else:
                cmd = './dummy --gpu 1 --network %s --layer_csv %s --trial 1000000' % (NET, OUTNAME)
            shcom(cmd)
