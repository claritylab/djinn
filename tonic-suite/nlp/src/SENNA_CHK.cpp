...
int* SENNA_CHK_forward(SENNA_CHK *chk, const int *sentence_words, const int *sentence_caps, const int *sentence_posl, TonicSuiteApp app)
{
  chk->input_state = SENNA_realloc(chk->input_state, sizeof(float), (app.pl.num+chk->window_size-1)*(chk->ll_word_size+chk->ll_caps_size+chk->ll_posl_size));
  chk->output_state = SENNA_realloc(chk->output_state, sizeof(float), app.pl.num*chk->output_state_size);
  ...
  app.pl.data = (char*)
    malloc(app.pl.num*(chk->window_size*input_size)*sizeof(float));

  for(int idx = 0; idx < app.pl.num; idx++){
      memcpy((char*)(app.pl.data+idx*(chk->window_size)*(input_size)*sizeof(float)),
  ...
  if(app.djinn) {
    ...
    SOCKET_receive(app.socketfd,
        ...
        chk->output_state_size*sizeof(float)*app.pl.num,
        debug
        );
  ...
  SENNA_nn_viterbi(chk->labels, chk->viterbi_score_init, chk->viterbi_score_trans, chk->output_state, chk->output_state_size, app.pl.num);
