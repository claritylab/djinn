...
int* SENNA_POS_forward(SENNA_POS *pos, const int *sentence_words, const int *sentence_caps, const int *sentence_suff, TonicSuiteApp app)
{
  pos->input_state = SENNA_realloc(pos->input_state,
      sizeof(float),
      (app.pl.num+pos->window_size-1)*(pos->ll_word_size+pos->ll_caps_size+pos->ll_suff_size)
      );
  ...
  app.pl.data = (char*) malloc(app.pl.num*(pos->window_size*(pos->ll_word_size+pos->ll_caps_size+pos->ll_suff_size))*sizeof(float));

  for(int idx = 0; idx < app.pl.num; idx++){
    memcpy((char*)(app.pl.data+idx*(pos->window_size)*(pos->ll_word_size+pos->ll_caps_size+pos->ll_suff_size)*sizeof(float)),
  ...
  if(app.djinn) {
    SOCKET_send(app.socketfd,
        (char*)app.pl.data,
        app.pl.num*app.pl.size*sizeof(float),
        debug
        );
