...
int* SENNA_NER_forward(SENNA_NER *ner, const int *sentence_words, const int *sentence_caps, const int *sentence_gazl, const int *sentence_gazm, const int *sentence_gazo, const int *sentence_gazp, TonicSuiteApp app)
{
  int input_size = ner->ll_word_size+ner->ll_caps_size+ner->ll_gazl_size+ner->ll_gazm_size+ner->ll_gazo_size+ner->ll_gazp_size;

  ner->input_state = SENNA_realloc(ner->input_state, sizeof(float), (app.pl.num+ner->window_size-1)*input_size);
  ner->output_state = SENNA_realloc(ner->output_state, sizeof(float), app.pl.num*ner->output_state_size);
  ...
  app.pl.data = (char*) malloc(app.pl.num*(ner->window_size*input_size)*sizeof(float));

  for(int idx = 0; idx < app.pl.num; idx++){
    memcpy((char*)(app.pl.data+idx*(ner->window_size)*(input_size)*sizeof(float)),
        (char*)(ner->input_state+idx*input_size),
        ner->window_size*input_size*sizeof(float));               
  }

  if(app.djinn) {
    SOCKET_send(app.socketfd,
        app.pl.data, 
        app.pl.num*(ner->window_size*input_size*sizeof(float)),
        ner->debug
        );
    SOCKET_receive(app.socketfd,
        (char*)(ner->output_state),
  ...
  SENNA_nn_viterbi(ner->labels,
                   ner->viterbi_score_init,
                   ner->viterbi_score_trans,
                   ner->output_state,
                   ner->output_state_size,
                   app.pl.num);
