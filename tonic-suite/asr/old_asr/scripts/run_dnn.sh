...
# Config:
gmmdir=exp/tri3b
data_fmllr=exp/data-fmllr-tri4b
train_set=train
test_set=test
...
if [ $stage -le 0 ]; then
  ...
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     ...
     $dir data/$test_set $gmmdir $dir/log $dir/data || exit 1
  # train
  dir=$data_fmllr/$train_set
  steps/nnet/make_fmllr_feats.sh --nj 10 --cmd "$train_cmd" \
     --transform-dir ${gmmdir}_ali \
     $dir data/$train_set $gmmdir $dir/log $dir/data || exit 1
...
if [ $stage -le 2 ]; then
  ...
  $cuda_cmd $dir/log/train_nnet.log \
    ...
    $data_fmllr/${train_set}_tr90 $data_fmllr/${train_set}_cv10 data/lang $ali $ali $dir || exit 1;
  # Decode (reuse HCLG graph)
  steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    $gmmdir/graph $data_fmllr/$test_set $dir/decode_bd_tgpr_${test_set} || exit 1;
fi
...
if [ $stage -le 6 ]; then
  ...
  for ITER in 1 2 3 4; do
    steps/nnet/decode.sh --nj 10 --cmd "$decode_cmd" --config conf/decode_dnn.config \
      --nnet $dir/${ITER}.nnet --acwt $acwt \
      $gmmdir/graph $data_fmllr/${test_set} $dir/decode_bd_tgpr_${test_set}_iter${ITER} || exit 1;
  done 
fi
