...
// broken
void test_reshape(int in_elts, float* in, float* out, Net<float>* espresso)
{
    ...
    if(in_elts/(c_in*w_in*h_in) > n_in)
    {
        n_in = in_elts/(c_in*w_in*h_in);
        printf("Reshaping input to dims %d %d %d %d...\n", n_in, c_in, w_in, h_in);
        espresso->input_blobs()[0]->Reshape(n_in, c_in, w_in, h_in);
        float *tmp = realloc(in, espresso->input_blobs()[0]->count() * sizeof(float));
        if(tmp != NULL)
            in = tmp;
        else {
        ...
        tmp = realloc(out, espresso->output_blobs()[0]->count() * sizeof(float));
        if(tmp != NULL)
            out = tmp;
        else {
            printf("Can't realloc\n");
...
int request_thread_init(ThreadParams in)
{
    ...
    pthread_t thread_id;
    if(pthread_create(&thread_id, &attr, request_handler, &in) != 0){
...
void* request_handler(void* inc)
{
    ...
    int req_type;
    SOCKET_receive(t->sock, (char*)&req_type, sizeof(int), t->debug);  
    ...
    int sock_elts = SOCKET_rxsize(t->sock);
    if(sock_elts < 0){
    ...
    // broken
    // test_reshape(sock_elts, in, out, espresso);
    ...
    while(1)
    {
        if(t->debug) printf("Receiving input features from client...\n");
        int rcvd = SOCKET_receive(t->sock, (char*) in, in_elts*sizeof(float), t->debug);
        if(rcvd == 0) break; // Client closed the socket

        if(t->debug) printf("Start neural network forward pass...\n");
        ...
        if(t->debug) printf("Sending result back to client...\n");
        SOCKET_send(t->sock, (char*) out, out_elts*sizeof(float), t->debug);
    }
    ...
    if(t->debug) printf("Socket closed by the client. Terminating thread now.\n");
