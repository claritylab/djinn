...
def main(args):
    ...
    with open(input_file, 'rb') as f:
         ...
         for row in data:
               ...
               # calculate out_dim
               cur_in_dim = float(row['height'])
               cur_kernel_size = float(row['kernel_size'])
               cur_stride = float(row['stride'])
               out_dim.append((cur_in_dim - cur_kernel_size)/cur_stride + 1.0)
    
    in_dim = np.multiply(np.multiply(channel, height), width)
    
    out_dim =  np.multiply(np.multiply(out_dim, out_dim), channel)
    ...
    csv_line = 'pool,log,a*log(b*x0)+c*log(d*x1)+e,5,'
