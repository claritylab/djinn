...
def main(args):
    ...
    with open(input_file, 'rb') as f:
         ...
         for row in data:
               ...
               in_dim.append(float(row['height']) * float(row['width']) * float(row['channel']))
               local_size.append(float(row['local_size']))
    
    
    x_data = in_dim
    #x_data = [np.asarray(in_dim), np.asarray(local_size)]
    y_data = gflops
    ...
    csv_line = 'norm,linear,a*x+b,2,'+str(slope)+','+str(intercept)+','+str(std_err)+','+str(r_value**2)
